...
class ModularJsonDatabase {
    constructor() {
        ...
        // Priority: AEMS_DATA_DIR (Electron) > DATA_DIR (env) > default (./data)
    ...
    /**
     * Determine the data directory based on environment variables and mode
     * Uses the EnvironmentDetector utility for consistency
     */
    getDataDirectory() {
        const dataDir = EnvironmentDetector.getDataDirectory();
        const mode = EnvironmentDetector.getMode();
        console.log(`📁 Using ${mode} data directory: ${dataDir}`);
        return dataDir;
    }
