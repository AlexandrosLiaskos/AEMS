...
class GmailService {
    ...
    reinitializeOAuth() {
        // Reload environment variables from the settings file
        if (process.env.AEMS_MODE === 'desktop') {
            const DesktopSettings = require('./desktop-settings');
            const settingsManager = new DesktopSettings();
            settingsManager.loadEnv(); // This should reload the .env file
        }

        // Reinitialize the OAuth client with new credentials
        this.initializeOAuth();
    }
