...
class EnvironmentValidator {
    ...
    validateOrExit() {
        ...
        // Handle errors - but don't exit in desktop mode
        if (!result.isValid) {
            if (process.env.AEMS_MODE === 'desktop') {
                console.log('📝 Desktop mode: Some features will be limited until API keys are configured');
                result.errors.forEach(error => console.log(`  ℹ️  ${error}`));
                console.log('✅ Application starting with limited functionality');
            } else {
