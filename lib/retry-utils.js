...
class RetryUtils {
    ...
    async withRetry(fn, options = {}) {
        ...
        const scheduler = require('./request-scheduler');
        const useScheduler = process.env.NODE_ENV === 'production';
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                // Optionally schedule to even out requests in production
                const exec = () => fn();
                const result = throttleKey && useScheduler
                    ? await scheduler.schedule(exec, throttleKey)
                    : await exec();
                return result;
            } catch (error) {
                lastError = error;
                ...
                // Add jitter to backoff delay in production
                const jitterPct = Math.min(Math.max(parseFloat(process.env.BACKOFF_JITTER_PCT || '0.2'), 0), 1);
                const jitter = useScheduler ? (Math.random() * 2 - 1) * jitterPct : 0;
                const wait = Math.max(0, Math.round(delay * (1 + jitter)));
